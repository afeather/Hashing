package SHA2;
import SHA.SHA;

/*
 * TO DO
 * Need to fix message length - messagelength2 is never used.
 */

public class SHA512 extends SHA {
	
	public final long[] k = { 
			0x428a2f98d728ae22L, 0x7137449123ef65cdL, 0xb5c0fbcfec4d3b2fL, 0xe9b5dba58189dbbcL, 0x3956c25bf348b538L, 
	        0x59f111f1b605d019L, 0x923f82a4af194f9bL, 0xab1c5ed5da6d8118L, 0xd807aa98a3030242L, 0x12835b0145706fbeL, 
	        0x243185be4ee4b28cL, 0x550c7dc3d5ffb4e2L, 0x72be5d74f27b896fL, 0x80deb1fe3b1696b1L, 0x9bdc06a725c71235L, 
	        0xc19bf174cf692694L, 0xe49b69c19ef14ad2L, 0xefbe4786384f25e3L, 0x0fc19dc68b8cd5b5L, 0x240ca1cc77ac9c65L, 
	        0x2de92c6f592b0275L, 0x4a7484aa6ea6e483L, 0x5cb0a9dcbd41fbd4L, 0x76f988da831153b5L, 0x983e5152ee66dfabL, 
	        0xa831c66d2db43210L, 0xb00327c898fb213fL, 0xbf597fc7beef0ee4L, 0xc6e00bf33da88fc2L, 0xd5a79147930aa725L, 
	        0x06ca6351e003826fL, 0x142929670a0e6e70L, 0x27b70a8546d22ffcL, 0x2e1b21385c26c926L, 0x4d2c6dfc5ac42aedL, 
	        0x53380d139d95b3dfL, 0x650a73548baf63deL, 0x766a0abb3c77b2a8L, 0x81c2c92e47edaee6L, 0x92722c851482353bL, 
	        0xa2bfe8a14cf10364L, 0xa81a664bbc423001L, 0xc24b8b70d0f89791L, 0xc76c51a30654be30L, 0xd192e819d6ef5218L, 
	        0xd69906245565a910L, 0xf40e35855771202aL, 0x106aa07032bbd1b8L, 0x19a4c116b8d2d0c8L, 0x1e376c085141ab53L, 
	        0x2748774cdf8eeb99L, 0x34b0bcb5e19b48a8L, 0x391c0cb3c5c95a63L, 0x4ed8aa4ae3418acbL, 0x5b9cca4f7763e373L, 
	        0x682e6ff3d6b2b8a3L, 0x748f82ee5defb2fcL, 0x78a5636f43172f60L, 0x84c87814a1f0ab72L, 0x8cc702081a6439ecL, 
	        0x90befffa23631e28L, 0xa4506cebde82bde9L, 0xbef9a3f7b2c67915L, 0xc67178f2e372532bL, 0xca273eceea26619cL, 
	        0xd186b8c721c0c207L, 0xeada7dd6cde0eb1eL, 0xf57d4f7fee6ed178L, 0x06f067aa72176fbaL, 0x0a637dc5a2c898a6L, 
	        0x113f9804bef90daeL, 0x1b710b35131c471bL, 0x28db77f523047d84L, 0x32caab7b40c72493L, 0x3c9ebe0a15c9bebcL, 
	        0x431d67c49c100d4cL, 0x4cc5d4becb3e42b6L, 0x597f299cfc657e2aL, 0x5fcb6fab3ad6faecL, 0x6c44198c4a475817L
	};
	
	public long[] hash = new long[8];
	public long messagelength1 = 0, messagelength2 = 0;
	
	public long[] buffer = new long[16];
	public int buffercount = 0;
	
	public SHA512() { reset(); }

	public void update(byte b) {
		int index = buffercount / 8;
		int offset = buffercount % 8;
		
		buffer[index] = buffer[index] | (long)(b & 0xFF) << ((7 - offset) * 8);
		
		buffercount++;
		messagelength2 += 8;
		
		if (buffercount == 128) processBuffer();
	}

	public void processBuffer() {
	
		long[] w = new long[80], temp = hash.clone();
		long S0, S1, s1, ch, temp1, s0, maj, temp2;
		
		System.arraycopy(buffer, 0, w, 0, 16);
		
		for (int i = 0; i < 16; i++) {
			
			s1    = (Long.rotateRight(temp[4], 14)) ^ (Long.rotateRight(temp[4], 18)) ^ (Long.rotateRight(temp[4], 41));
			ch    = (temp[4] & temp[5]) ^ ((~temp[4]) & temp[6]);
			temp1 =  temp[7] + s1 + ch + k[i] + w[i];
			s0    = (Long.rotateRight(temp[0], 28)) ^ (Long.rotateRight(temp[0], 34)) ^ (Long.rotateRight(temp[0], 39));
			maj   = (temp[0] & temp[1]) ^ (  temp[0]  & temp[2]) ^ (temp[1] & temp[2]);
			temp2 =  s0 + maj;
			
			temp[7] =  temp[6];
			temp[6] =  temp[5];
			temp[5] =  temp[4];
			temp[4] = (temp[3] + temp1) & 0xFFFFFFFFFFFFFFFFL;
			temp[3] =  temp[2];
			temp[2] =  temp[1];
			temp[1] =  temp[0];
			temp[0] = (temp1   + temp2) & 0xFFFFFFFFFFFFFFFFL;
		}
		
		for (int i = 16; i < 80; i++) {
			
			S0 = (Long.rotateRight(w[i-15], 1 )) ^ (Long.rotateRight(w[i-15], 8)) ^ (w[i-15] >>> 7 );
			S1 = (Long.rotateRight(w[i-2 ], 19)) ^ (Long.rotateRight(w[i-2 ], 61)) ^ (w[i-2 ] >>> 6);
			
			w[i] = w[i-16] + S0 + w[i-7] + S1;
			
			s1    = (Long.rotateRight(temp[4], 14)) ^ (Long.rotateRight(temp[4], 18)) ^ (Long.rotateRight(temp[4], 41));
			ch    = (temp[4] & temp[5]) ^ ((~temp[4]) & temp[6]);
			temp1 =  temp[7] + s1 + ch + k[i] + w[i];
			s0    = (Long.rotateRight(temp[0], 28)) ^ (Long.rotateRight(temp[0], 34)) ^ (Long.rotateRight(temp[0], 39));
			maj   = (temp[0] & temp[1]) ^ (  temp[0]  & temp[2]) ^ (temp[1] & temp[2]);
			temp2 =  s0 + maj;
			
			temp[7] =  temp[6];
			temp[6] =  temp[5];
			temp[5] =  temp[4];
			temp[4] = (temp[3] + temp1) & 0xFFFFFFFFFFFFFFFFL;
			temp[3] =  temp[2];
			temp[2] =  temp[1];
			temp[1] =  temp[0];
			temp[0] = (temp1   + temp2) & 0xFFFFFFFFFFFFFFFFL;
			
		}
		
		for (int i = 0; i < 8; i++) hash[i] = (hash[i] + temp[i]) & 0xFFFFFFFFFFFFFFFFL;
		
		buffercount = 0;
		
	}

	public void addPadding() {
		
		buffer[buffercount / 8] = buffer[buffercount / 8] | ((long) (0x80) << ( (7 - buffercount % 8) * 8));
		buffercount++;
		
		if (buffercount > 112) {	
			
			while (buffercount % 8 > 0) { 
				buffer[buffercount / 8] = buffer[buffercount / 8] | ((long) (0x00) << ( (7 - buffercount % 8) * 8));
				buffercount++;
			}
			
			while (buffercount < 128 ) {
				buffer[buffercount / 8] = (long) 0;
				buffercount += 8;
			}

			processBuffer();
		}
		
		if (buffercount < 112) {
			
			while (buffercount % 8 > 0) { 
				buffer[buffercount / 8] = buffer[buffercount / 8] | ((long) (0x00) << ( (7 - buffercount % 8) * 8));
				buffercount++;
			}
			
			while (buffercount < 112 ) {
				buffer[buffercount / 8] = (long) 0;
				buffercount += 8;
			}
			
		}
		
		buffer[14] = (long)(messagelength1);
		buffer[15] = (long)(messagelength2);
		
		processBuffer();
	}

	public byte[] getHash() {
		addPadding();
		
		byte[] digest = new byte[64];
		long nextlong;
		
		for (int i = 0; i < 8; i++) {
			nextlong = hash[i];
			
			digest[i*8    ] = (byte)((nextlong >>> 56) & 0xFF);
			digest[i*8 + 1] = (byte)((nextlong >>> 48) & 0xFF);
			digest[i*8 + 2] = (byte)((nextlong >>> 40) & 0xFF);
			digest[i*8 + 3] = (byte)((nextlong >>> 32) & 0xFF);
			digest[i*8 + 4] = (byte)((nextlong >>> 24) & 0xFF);
			digest[i*8 + 5] = (byte)((nextlong >>> 16) & 0xFF);
			digest[i*8 + 6] = (byte)((nextlong >>> 8 ) & 0xFF);
			digest[i*8 + 7] = (byte)((nextlong       ) & 0xFF);
		}
		
		reset();
		
		return digest; 
	}

	public void reset() {
		hash[0] = 0x6a09e667f3bcc908L;
		hash[1] = 0xbb67ae8584caa73bL;
		hash[2] = 0x3c6ef372fe94f82bL;
		hash[3] = 0xa54ff53a5f1d36f1L;
		hash[4] = 0x510e527fade682d1L;
		hash[5] = 0x9b05688c2b3e6c1fL;
		hash[6] = 0x1f83d9abfb41bd6bL;
		hash[7] = 0x5be0cd19137e2179L;
		
		buffercount = 0;
		messagelength2 = 0;
		messagelength1 = 0;
	}
	
	public class SHA384 extends SHA512 {
		
		public byte[] getHash() {
			addPadding();
			
			byte[] digest = new byte[48];
			long nextlong;
			
			for (int i = 0; i < 6; i++) {
				nextlong = hash[i];
				
				digest[i*8    ] = (byte)((nextlong >>> 56) & 0xFF);
				digest[i*8 + 1] = (byte)((nextlong >>> 48) & 0xFF);
				digest[i*8 + 2] = (byte)((nextlong >>> 40) & 0xFF);
				digest[i*8 + 3] = (byte)((nextlong >>> 32) & 0xFF);
				digest[i*8 + 4] = (byte)((nextlong >>> 24) & 0xFF);
				digest[i*8 + 5] = (byte)((nextlong >>> 16) & 0xFF);
				digest[i*8 + 6] = (byte)((nextlong >>> 8 ) & 0xFF);
				digest[i*8 + 7] = (byte)((nextlong       ) & 0xFF);
			}
			
			reset();
			
			return digest; 
		}
		
		public void reset() {
			hash[0] = 0xcbbb9d5dc1059ed8L;
			hash[1] = 0x629a292a367cd507L;
			hash[2] = 0x9159015a3070dd17L;
			hash[3] = 0x152fecd8f70e5939L;
			hash[4] = 0x67332667ffc00b31L;
			hash[5] = 0x8eb44a8768581511L;
			hash[6] = 0xdb0c2e0d64f98fa7L;
			hash[7] = 0x47b5481dbefa4fa4L;
			
			buffercount = 0;
			messagelength2 = 0;
			messagelength1 = 0;
		}
		
	}
	
}
